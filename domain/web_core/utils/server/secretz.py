...
class secretz:
    ...
    def _new_token(max_byte: int = 128) -> str:
        """ generate new token urlsafe """
        return secrets.token_urlsafe(nbytes=max_byte)
    
    def set_to_dotenv(variable_name: str, value_of_variable: str, dotenv_path: str = ".env",) -> Tuple[bool, str]:
...
def get_secret_key_server():
    ...
    if key is None:
        secretz.set_to_dotenv(variable_name="secret_key", value_of_variable=secretz.generate_new_token_urlsafe())
