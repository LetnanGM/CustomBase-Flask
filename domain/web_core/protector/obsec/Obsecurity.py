...
from ...data.configuration.security.config_obsecurity import (
    default_config, server_fake, fake_headers_config, honeypot_routes,
    bad_ua_patterns
)
from ...bootstrap import obs_logger
    ...
    def obsec_routes(self):
        ...
        def security_checks():
            ...
            if client_ip in self.blocked_ips:
                obs_logger.vsilent(f"⛔ BLOCKED IP attempted access: {client_ip} -> {request.path}")
                abort(403)
            ...
            if self.config['enable_ua_blocking']:
                ...
                if self._is_bad_ua(ua):
                    obs_logger.vsilent(f"🚨 BAD UA detected: {ua[:80]}... from {client_ip}")
        ...
        if self.config['enable_honeypot']:
            def honeypot_handler():
                ...
                obs_logger.vsilent(f"🍯 HONEYPOT triggered: {request.path} | Method: {request.method} | IP: {client_ip}")
                ...
                if hit_count >= self.config['auto_block_threshold']:
                    ...
                    obs_logger.vsilent(f"🔒 IP AUTO-BLOCKED: {client_ip} ({hit_count} honeypot hits)")
                else:
                    obs_logger.vsilent(f"📊 Honeypot hit #{hit_count} for fingerprint: {fingerprint[:16]}...")
