...
class guardian:
    ...
    def setup(app: Flask):
        def handle_exception(exc_type: Any, exc_value: Any, exc_traceback: Any):
            ...
            message = ">>> UNHANDLED EXCEPTION CAUGHT (Server still running) <<<\n" + "".join(traceback.format_exception(exc_type, exc_value, exc_traceback))
            ...
            def handle_flask_error(e):
                GuardianLogger.error(f">>> FLASK ERROR, SERVER STILL RUNNING <<< \n{e}\n>>> END OF REPORT ERROR <<<")
                GuardianLogger.vsilent(f">>> FLASK ERROR, SERVER STILL RUNNING <<< \n{e}\n>>> END OF REPORT ERROR <<<")
                return "You found some bug in our application!, the bug will reported into server developer!\nThanks for your report", 500
