...
class Compute:
    ...
    def top_cpu(self) -> str:
        return sorted(psutil.process_iter(["pid", "name", "cpu_percent"]),
    ...
    def top_ram(self) -> list:
        return sorted(psutil.process_iter(["pid", "name", "memory_percent"]), 
            ...
            try:
                ...
                partitions = {
                    part.device: {
                        ...
                        "warn": {
                            "status": warn,
                            "message": [f"WARNING: Disk {part.mountpoint} >80!!!" if warn else None]
                        }
                    }
                }
    ...
    def disk_io(self) -> dict:
        ...
        return {
            "read": {self._gb(io.read_bytes)},
            "write": {self._gb(io.write_bytes)}
        }
